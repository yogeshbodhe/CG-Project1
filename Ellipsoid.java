...
public class Ellipsoid extends UVShape
{
    ...
    {
        p.x = ax.value * Math.cos(u) * Math.cos(v);
        p.y = ay.value * Math.sin(u) * Math.cos(v);
        p.z = az.value * Math.sin(v);
    }
    
    // Compute normal vector
    public void evalNormal(double u, double v, Vector3d n)
    {
    	n.x = ay.value * az.value * Math.cos(u) * Math.cos(v);
        n.y = ax.value * az.value * Math.sin(u) * Math.cos(v);
        n.z = ax.value * ay.value * Math.sin(v);
    }
}
